...
parser.add_argument('--num_gpus',   type=int,  default=2,       help='How many GPUs to use.')
...
def train():
  ...
  # number of batches processed * NUMGPUS.
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    batch_queue    = tf.contrib.slim.prefetch_queue.prefetch_queue([images, labels], capacity=2 * NUMGPUS)
    tower_grads    = []
      ...
      for i in xrange(NUMGPUS):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (CIFAR10.TOWER_NAME, i)) as scope:
            ...
            # This functionconstructs the entire CIFAR model but shares the variables across all towers.
    ...
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=FLAGS.log_device))
    sess.run(init)
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      if step % 1000 == 0:
        num_examples_per_step = FLAGS.batch_size * NUMGPUS
        examples_per_sec      = num_examples_per_step / duration
        sec_per_batch         = duration / NUMGPUS

        format_str = ('%s: step %d, loss = %.2f (%.1f examples/sec; %.3f sec/batch)')
        print (format_str % (datetime.now(), step+1000, loss_value, examples_per_sec, sec_per_batch))
